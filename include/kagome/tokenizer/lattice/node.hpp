...
constexpr const char* to_string(NodeClass cls) noexcept {
    switch (cls) {
        case NodeClass::Dummy: return "DUMMY";
        case NodeClass::Known: return "KNOWN";
        case NodeClass::Unknown: return "UNKNOWN";
        case NodeClass::User: return "USER";
...
public:
    ...
    [[nodiscard]] NodeClass node_class() const noexcept { return class_; }
    [[nodiscard]] std::int32_t cost() const noexcept { return cost_; }
    [[nodiscard]] std::int16_t left_id() const noexcept { return left_id_; }
    [[nodiscard]] std::int16_t right_id() const noexcept { return right_id_; }
    [[nodiscard]] std::int16_t weight() const noexcept { return weight_; }
    [[nodiscard]] const std::string& surface() const noexcept { return surface_; }
    [[nodiscard]] const Node* prev() const noexcept { return prev_; }
    
    // Mutators
    void set_id(std::int32_t id) noexcept { id_ = id; }
    void set_position(std::int32_t position) noexcept { position_ = position; }
    void set_start(std::int32_t start) noexcept { start_ = start; }
    void set_class(NodeClass cls) noexcept { class_ = cls; }
    void set_cost(std::int32_t cost) noexcept { cost_ = cost; }
    void set_left_id(std::int16_t left_id) noexcept { left_id_ = left_id; }
    void set_right_id(std::int16_t right_id) noexcept { right_id_ = right_id; }
    void set_weight(std::int16_t weight) noexcept { weight_ = weight; }
    void set_surface(std::string surface) { surface_ = std::move(surface); }
    void set_prev(const Node* prev) noexcept { prev_ = prev; }
